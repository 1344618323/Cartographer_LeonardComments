...
// (cxn)sensor_msgs::LaserScan的range[i]的数据类型是float.
...
LaserScanToPointCloudWithIntensities(const LaserMessageType& msg) {
  ...
  for (size_t i = 0; i < msg.ranges.size(); ++i) {
    ...
    if (HasEcho(echoes)) {
      //(cxn)这种多回波雷达，对一个方向可能会返回多个数据，如激光找到玻璃上，
      //可能会返回两个值 到玻璃的距离、到墙的距离，这些值看起来是无序放到echo中的，还不确定
  ...
  //(cxn)激光点的时间戳形式为 从 -0.1s ～ 0s
  //(cxn)timestamp是指 最后一束激光的记录时间
