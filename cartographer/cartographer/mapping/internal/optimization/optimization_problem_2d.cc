...
void OptimizationProblem2D::Solve(
  ...
  for (const auto& submap_id_data : submap_data_) {
    ...
    if (first_submap || frozen) {
      ...
      // trajectory.
      //第一帧submap或者固定轨迹中的submap的位姿不做优化
  ...
  // 添加submap与node之间的约束，回环的约束要加核函数平滑下
...
//找到输入参数time时刻前后的odom位姿，并通过插值得到time时刻的odom位姿
...
OptimizationProblem2D::CalculateOdometryBetweenNodes(
  ...
  if (odometry_data_.HasTrajectory(trajectory_id)) {
    ...
    // 输出的变换为 (Todom_A * Rimu_A')' * (Todom_B*Rimu_B')
    // 考虑到spa的优化中node用的位姿 就是 Tlw_A-2d = Tlw_A * Rimu_A'，这里后面右乘一个Rimu_A 是对的
