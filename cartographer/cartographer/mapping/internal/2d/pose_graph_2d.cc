...
/*
用于给优化问题新增submap，主要设置新增submap的全局位姿的初始值，会优化的
用优化问题中的上一个submap的全局位姿来推理：Tgw_submap1 * Tlw_submap1' * Tlw_submap2
*/
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
  ...
  if (insertion_submaps.size() == 1) {
    ...
    if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0) {
      ...
      //(cxn)initial-trajectory-poses-这个变量先不管
  ...
  //(cxn)把EndOf-Trajectory当做 std::map中的end()就是了
  ...
  if (submap_data_.at(last_submap_id).submap == insertion_submaps.front()) {
    ...
    //说明此时的有新的要加入优化问题的submap
    const auto& first_submap_pose = submap_data.at(last_submap_id).global_pose;
    optimization_problem_->AddSubmap(
        trajectory_id,
        // Tgw_submap1 * Tlw_submap1' * Tlw_submap2
        first_submap_pose *
  ...
  //程序能运行到这一步，说明’last-submap-id’正是‘insertion-submaps.back()’，submap没来新的
...
//这个函数在成员变量中 trajectory-nodes-加入新node、在submap-data-加入新submpa（如果有的话），
//并给任务调度器加任务：给node和submap计算约束
NodeId PoseGraph2D::AddNode(
    std::shared_ptr<const TrajectoryNode::Data> constant_data,
    const int trajectory_id,
    const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps) {
  
  //对于一个新的node有 Tgw-submap*Tlw-submap'*Tlw_node = Tgw_node
  //这个subamp是node所在轨迹优化问题中最新优化的submap
  ...
  //(cxn) 当发现该轨迹有了新的submap，就在变量submap-data-插入新的submap （用的是智能指针哦）
  ...
  // execute the lambda.
  // 记录下插入的submap中的第一个是不是已经完成的submap
  ...
  if (!global_localization_samplers_[trajectory_id]) {
    global_localization_samplers_[trajectory_id] =
        common::make_unique<common::FixedRatioSampler>(
            ...
            //(cxn)2d.lua globalsamplingratio 0.003
  }
}
...
void PoseGraph2D::ComputeConstraint(const NodeId& node_id,
  ...
  //计算node所属轨迹与submap所属轨迹上次绑定在一起的时间
  ...
  if (node_id.trajectory_id == submap_id.trajectory_id ||
    ...
    // local search window.
    // 若submap与node在同一轨迹上 or submap的轨迹与node的轨迹在不久前刚刚绑定，则进入这个条件
    // 给分枝定界scan-match设定初值，初值为 Tgw-submap'*Tgw-node-2d （2d代表少了一个imu中的yaw角）
...
//只有是 {这个刚完成的submap 与 不在这个subamap中的node} 这样的组合时，才会去找约束
...
/*设置新加入的node与submap（如果有的话）在优化问题中的初值，并建立node与submap的约束
  同时发布任务：找node与已完成submap约束、找新完成submap与以前的node的约束
  累加一定数量的node之后，就spa优化
*/ 
void PoseGraph2D::ComputeConstraintsForNode(
  ...
  */
  ...
  /*给优化问题加入该node，并设置属性：
        包括node在local-frame中2d位姿：不变的
        包括node在global-frame中2d位姿：设置一个初值，会优化的。初始值用该node所在submap在优化问题中优化值来推理：Tgw-node-2d = Tgw-submap1 * Tlw-submap1' * Tlw-node-2d
        node在imu-frame中的旋转：不变的
  */
  //算回了原来localslam中ceres优化后的2d结果 Tlw-node*Rg' = Tlw-node-2d （注意此时是少了一个IMU中的yaw角的）
  const transform::Rigid2d local_pose_2d = transform::Project2D(
      constant_data->local_pose *
      transform::Rigid3d::Rotation(constant_data->gravity_alignment.inverse()));
  
  //Tgw-node-2d = Tgw-submap1 * Tlw-submap1' * Tlw-node-2d
  ...
  for (size_t i = 0; i < insertion_submaps.size(); ++i) {
    ...
    //新增约束：即优化中边的观测值 Tlw-submap‘*Tlw-node-2d
    //spa公式： Tij‘ * Ti' *Tj：在cartographer中，Tsubmap-node_2d就是 Tij，就是边的观测值
    //Ti就是 Tgw-submap；Tj就是 Tgw-node    
  ...
  //从所有完成的submap中，找新node与submap约束，另外这些已完成的submap一定没有当前的node
  ...
  //在pose-graph.lua的默认配置中，每增加90个node，就进行一次优化
...
//算submap-id（已完成的） 中最后一帧node的时间 与 node-id 时间的 较大值
...
void PoseGraph2D::HandleWorkQueue(
  ...
  //这个循环的一个用途是在 pure定位中 减少新轨迹的submap数量
...
void PoseGraph2D::RunOptimization() {
  ...
  for (const int trajectory_id : node_data.trajectory_ids()) {
    ...
    //修正参与到spa中的 pose-graph中 node 的位姿
    //（这里说的pose-graph中的指的是 pose-graph.cc中 对象的成员，与optimization-problem.cc中的成员变量区分，下同）
    ...
    //算出spa优化求解时最新的submap的 Ｔgw-submap*Tlw-submap'
    const auto local_to_new_global =
        ComputeLocalToGlobalTransform(submap_data, trajectory_id);

    //算出pose-graph中 global-submap-poses- 最新的submap的 Ｔoldgw-submap*Tlw-submap'，也就是未spa优化之前的结果
    //注意哦，global-submap-poses- 最新的submap 可能不是 spa优化求解时最新的submap
    //（spa优化求解时最新的submap） 可能比 （global-submap-poses- 最新的submap）更新或是同一个
    ...
    //将未参与到spa中的 pose-graph中 node的全局位姿修正
...
//这个函数用来计算　参数’global_submap_poses‘　在　参数'trajectory_id‘轨迹中最后一个submap的 Tgw-submap*Tlw-submap'　
//如果 参数’global_submap_poses‘还没有数据，就返回 Eye
...
// 将这个submap 清了； 将源生于这个submap的 node 清了； 将这些node与其他submap的 约束 也清了；
void PoseGraph2D::TrimmingHandle::MarkSubmapAsTrimmed(
    ...
    for (const Constraint& constraint : parent_->constraints_) {
      if (constraint.submap_id == submap_id) {
        if (constraint.tag == Constraint::Tag::INTRA_SUBMAP &&
          ...
          // removed.
          // 搜集 与这个submap有 非回环约束 且 与其他submap 无非回环约束 的node 
  ...
  {
    // 将 有与其他submap有约束的node 中 （与这个submap有 非回环约束 且 与其他submap 无非回环约束 的node） 剔除
    // 说白了，就是将  源生于这个submap 的node 都清了
    // 将关于这些 （源生于这个submap 的node）的约束也清了
    std::vector<Constraint> constraints;
    for (const Constraint& constraint : parent_->constraints_) {
